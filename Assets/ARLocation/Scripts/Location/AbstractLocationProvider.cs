...
namespace ARLocation
{
    ...
    {
        private AngleLowPassFilter magneticHeadingLpFilter;
        private AngleLowPassFilter headingLpFilter;
        private bool useCompassLpFilter;
        ...
        public void SetCompassLowPassFactor(double factor)
        {
            if (factor > 0)
            {
                if (magneticHeadingLpFilter == null)
                {
                    magneticHeadingLpFilter = new AngleLowPassFilter(factor);
                    headingLpFilter = new AngleLowPassFilter(factor);
                }
                else
                {
                    magneticHeadingLpFilter.SetFactor(factor);
                    headingLpFilter.SetFactor(factor);
                }

                useCompassLpFilter = true;
            }
            else
            {
                magneticHeadingLpFilter = null;
                headingLpFilter = null;
                useCompassLpFilter = false;
            }
        }
