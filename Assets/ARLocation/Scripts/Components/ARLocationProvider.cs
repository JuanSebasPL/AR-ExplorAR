...
namespace ARLocation
{
    ...
    public class ARLocationProvider : Singleton<ARLocationProvider>
    {
        ...
        public LocationEnabledUnityEvent OnEnabled;

        [Tooltip("Called after each new location update.")]
        public LocationUpdatedUnityEvent OnLocationUpdated;

        [Tooltip("Called after each new raw device GPS data is obtained.")]
        public LocationUpdatedUnityEvent OnRawLocationUpdated;

        [Tooltip("Called after each new compass update.")]
        public CompassUpdatedUnityEvent OnCompassUpdated;
        ...
        private Action onRestartDelegate;

        private event LocationUpdatedDelegate OnLocationUpdatedDelegate;
        private event CompassUpdateDelegate OnCompassUpdateDelegate;
        ...
        Provider = new UnityLocationProvider();
            ...
            {
                Logger.LogFromMethod("ARLocationProvider", "Awake","Waiting for AR Tracking to start.", DebugMode);
                ARLocationManager.Instance.OnARTrackingStarted(InitProviderEventListeners);
            }
            else
            {
