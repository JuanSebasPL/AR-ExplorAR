...
namespace ARLocation
{
    ...
    public class GroundHeight : MonoBehaviour
    {
        ...
        private void ArPlaneManagerOnPlanesChanged(ARPlanesChangedEventArgs eventArgs)
        {
            ...
            // foreach (ARPlane plane in addedPlanes.Concat(updatedPlanes).ToList())
        ...
        private void ProcessPlane(ARPlane plane)
        {
            ...
            if (-plane.center.y > Settings.MaxGroundHeight || -plane.center.y < Settings.MinGroundHeight)
            {
        ...
        private void HitTestHandler(HitTestResult result)
        {
            ...
            if (!(height > Settings.MinGroundHeight) && (height < Settings.MaxGroundHeight))
            {
        ...
        private void UpdateObjectHeight()
        {
            ...
            {
                transform.position = MathUtils.SetY(transform.position, state.CurrentGroundY);
            }
        ...
        public void Update()
        {
            if (Settings.Smoothing <= 0) return;

            if (Mathf.Abs(transform.position.y - state.CurrentGroundY) <= Settings.Precision)
            {
                return;
            }
            ...
            var value = Mathf.Lerp(position.y, state.CurrentGroundY, t);
